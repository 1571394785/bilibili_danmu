...
import sys,win32gui,win32con,win32api,win32process,win32com.client
...
class MainDemo(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        # 添加一个本地网页
        self.add_new_tab(QUrl('http://121.43.33.137/docs/'), 'Local')
    ...
    def navigate_to_url(self):
        ...
        threading.Thread(target=self.set_default_background).start()
    ...
    def set_white_background(self):
        #调用SetLayeredWindowAttributes函数，将窗口的颜色设置为透明色
        #获取自身句柄
        hwnd = int(self.winId())
        win32gui.SetWindowLong(hwnd, -20, win32gui.GetWindowLong(hwnd, -20) | 0x80000)
        win32gui.SetLayeredWindowAttributes(hwnd, win32api.RGB(255, 255, 255), 100, 1)
    #把透明色复原
